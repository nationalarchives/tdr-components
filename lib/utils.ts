...
) => Promise<string[]> = async (componentPath) => {
  return await util
    .promisify(fs.readdir)(componentPath, { withFileTypes: true })
    .then((items) =>
...
) => Promise<{ [key: string]: any }> = async (componentName) => {
  const componentData = await getComponentData(componentName);
  return componentData.examples.reduce(
    (obj, example) => ({ ...obj, [example.name]: example.data }),
    {},
  );
};
...
export const renderHtml: (componentName: string, options: string) => string = (
  componentName,
  options,
) => {
...
const getComponentData: (
  componentName: string,
) => Promise<ComponentData> = async (componentName) => {
  const yamlPath = join(
    paths.componentPath,
    componentName,
    `${componentName}.yaml`,
  );
  const componentData: any = yaml.load(await readFile(yamlPath, "utf8"), {
    json: true,
  });
  return {
    name: componentName,
